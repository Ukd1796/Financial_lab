...
def _summarise_session(sess: dict) -> None:
    ...
    body = f"₹{starting_capital:,.0f} base · {sign}₹{pnl_abs:,.0f} ({sign}{pnl_pct:.1f}%) · {len(positions)} positions"
    if top:
    ...
    print(f"     {sid}: {sign}₹{pnl_abs:,.0f} ({sign}{pnl_pct:.1f}%) · {len(positions)} positions")
